...
def main(args):
    ...
    t2 = time.time()
    # 每次输入图像和商品的文字描述，输入空图像时退出
    while True:
        ima_path = input(f"image_path: ")
        if ima_path == "":
            break
        args.image_file = ima_path
        ...
        if image is not None:
            ...
            t4 = time.time()
        print(f"模型加载时间：{round(t2-t1,2)}s")
        print(f"模型推理总时间：{round(t4-t3,2)}s")
        print(f"GPU显存占用:{round(max_gpu_memory_cost / 1024 / 1024 / 1024,2)}GB")
